...
<title>Insert title here</title>
  ...
  function init(){
	  var sex = '<c:out value="${gdto.sung}"/>'
		document.mk.sung.value=sex;
	  
	  var kinds = '<c:out value="${gdto.kind}" />'
	    ...
	    var domain = '<c:out value="${gdto.email2}" />'
...
<input type="hidden" name="id" value="${gdto.id}">
  ...
  <input type="text" class="form-control" id="usr" name="title" value="${gdto.title}">
</div>
<div class="form-group">
  <label for="comment">내용</label>
  <textarea class="form-control" rows="5" name="content">${gdto.content}</textarea>
</div>
<div class="form-group">
...
<input type="text" name="email" value="${gdto.email}">@
<select name="email2">
