...
<ul class="pagination">
  	<li data-page=1>
		<c:if test="${pageObject.page > 1 }">
	  		<a href="${listURI }?ch=1&page=1&perPageNum=10&key=${pageObject.key }&word=${pageObject.word}"
	...
	<li data-page=${pageObject.startPage -1 }>
		<c:if test="${pageObject.startPage > 1 }">
	  		<a href="${listURI }?ch=1&page=${pageObject.startPage - 1 }&perPageNum=10&key=${pageObject.key }&word=${pageObject.word}"
	...
	<c:forEach begin="${pageObject.startPage }" end="${pageObject.endPage }" var="cnt">
  	<li ${(pageObject.page == cnt)?"class=\"active\"":"" } 
  	 data-page=${cnt }>
  	 	...
  	 	<c:if test="${pageObject.page != cnt }">
  			<a href="${listURI }?ch=1&page=${cnt }&perPageNum=10&key=${pageObject.key }&word=${pageObject.word}"
	...
	<c:if test="${pageObject.endPage < pageObject.totalPage }">
	  	<li data-page=${pageObject.endPage + 1 }>
	  		<a href="${listURI }?ch=1&page=${pageObject.endPage + 1 }&perPageNum=${pageObject.perPageNum}&key=${pageObject.key }&word=${pageObject.word}"
	...
	<c:if test="${pageObject.page < pageObject.totalPage }">
	  	<li data-page=${pageObject.totalPage }>
	  		<a href="${listURI }?ch=1&page=${pageObject.totalPage }&perPageNum=${pageObject.perPageNum}&key=${pageObject.key }&word=${pageObject.word}"
